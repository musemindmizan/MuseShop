    ...
    <main class="flex-1 overflow-y-auto p-6 bg-gray-100">
        ...
        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
            <div class="flex flex-col md:flex-row gap-4 justify-between">
                <form method="GET" action="{{route('admin.categories')}}" class="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                    ...
                    <select name="parent-category" onchange="this.form.submit()"
                        ...
                        @foreach ($parentCategories as $parentCategory)
                            <option value="{{$parentCategory->slug}}" {{request('parent-category') == $parentCategory->slug ? 'selected' : ''}}>{{$parentCategory->name}}</option>
                        @endforeach
                    </select>

                    @if ( request('search') || request('parent-category') )
                        <a href="{{route('admin.categories')}}" class="text-sm text-gray-500 hover:text-primary self-center">Clear</a>
                    @endif
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($categories as $category)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                @else
                                    <td class="px-6 py-4 text-sm text-purple-700">-</td>
                                @endif
                                <td class="px-6 py-4">
                                    <span
                                        class="bg-purple-50 text-purple-700 px-2 py-1 rounded text-xs font-semibold">0</span>
                                </td>
                                <td class="px-6 py-4">
