                    ...
                    <div class="swiper-slide flex items-center justify-center">
                        <div class="container mx-auto px-4 grid lg:grid-cols-2 gap-8 items-center h-full">
                            <div class="text-center lg:text-left space-y-4 animate-fadeIn">
                                <h2 class="text-4xl lg:text-6xl font-bold text-gray-800 leading-tight">{{ $slide->title }}</h2>
                                @if ($slide->subtitle)
                                    <p class="text-lg text-gray-600">{{ $slide->subtitle }}</p>
                                @endif
                                @if ($slide->button_text)
                                    <a href="{{ $slide->button_link ?: route('shop.index') }}"
                                        class="inline-block bg-primary text-white px-8 py-3 rounded hover:bg-blue-600 transition shadow-lg mt-4">{{ $slide->button_text }}</a>
                                @endif
                            </div>
                            <div class="hidden lg:block">
                                <img src="{{ asset('uploads/hero-slides/' . $slide->image) }}" alt="{{ $slide->title }}" class="mx-auto" />
                            </div>
                        </div>
                    </div>
                @empty
